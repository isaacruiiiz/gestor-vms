    ...
    <div className={`border rounded-lg p-4 shadow-sm transition-all duration-300 ease-out hover:-translate-y-1 ${cardStyle}`}>
      ...
      <div className="flex justify-between items-start mb-4">
        ...
        {usuarioActual ? (
          ...
          <span className="px-2 py-0.5 rounded text-[10px] uppercase font-bold bg-emerald-950 text-emerald-500 border border-emerald-900/50 tracking-wider transition-colors duration-300">
            Libre
          </span>
        )}
      </div>
      ...
      <div className="space-y-3">
        {usuarioActual ? (
          ...
          <div className="bg-black/40 p-3 rounded border border-zinc-800 transition-colors duration-300">
            ...
            <div className="flex items-center justify-between">
              ...
              <div className="flex items-center gap-3">
                <div className={`w-8 h-8 rounded-full flex items-center justify-center text-white text-xs font-bold shadow-sm ${usuarioActual.color} transform transition-transform hover:scale-110`}>
              ...
              {/* DERECHA: BOTÓN SALIR */}
              <button 
                onClick={liberarMaquina} 
                className="text-red-500 hover:text-red-400 text-xs font-semibold flex items-center gap-1 transition-all duration-200 cursor-pointer hover:scale-105 active:scale-95"
                aria-label="Liberar máquina"
              >
          ...
          <div className="flex gap-2">
            <select 
              className="flex-1 border border-zinc-800 rounded px-2 py-1.5 text-sm bg-zinc-950 text-zinc-300 focus:ring-1 focus:ring-emerald-500 focus:border-emerald-500 outline-none placeholder-zinc-600 font-sans appearance-none cursor-pointer transition-all duration-200 hover:border-zinc-600"
            ...
            <button 
              ...
              className="bg-emerald-700 hover:bg-emerald-600 text-white px-4 py-1 rounded text-sm font-medium transition-all duration-200 border border-emerald-600 cursor-pointer active:scale-95 hover:shadow-[0_0_10px_rgba(16,185,129,0.4)]"
            >
              Entrar
            </button>
          </div>
          ...
          <div className="mt-3 pt-3 border-t border-zinc-800 border-dashed">
            <div className="flex justify-between items-center mb-2">
              ...
              <div className="flex gap-1">
                 <select 
                  className="w-32 border border-zinc-800 rounded px-1 py-0.5 text-[10px] bg-zinc-950 text-zinc-400 outline-none font-sans cursor-pointer transition-colors duration-200 hover:border-zinc-600"
                  onChange={(e) => setUsuarioSeleccionado(e.target.value)}
                  value={usuarioSeleccionado}
                  style={{ fontFamily: 'inherit' }}
                ...
                <button 
                  onClick={unirseACola} 
                  title="Ponerme en cola" 
                  className="text-blue-500 hover:bg-zinc-800 p-1 rounded transition-all duration-200 cursor-pointer hover:scale-110 active:scale-90"
                  aria-label="Unirse a la cola"
                >
                  <UserPlus size={14} />
                </button>
              </div>
